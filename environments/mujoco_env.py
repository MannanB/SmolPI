...
class MujocoEnvironment(BaseEnvironment, ABC):
    def __init__(self, config: MujocoEnvConfig, control_freq_hz: float):
        ...
        xml_str = ""
        with open(self.config.xml_path, "r") as f:
            xml_str = f.read()
        ...
        self.obs_renderers = [mujoco.Renderer(self.model, width=cam.width, height=cam.height) for cam in self.config.observation_cams]

        # TODO: rename for clarity? these are only for rendering to mp4, not observations
        self.cam_renderers = [mujoco.Renderer(self.model, width=cam.width, height=cam.height) for cam in self.config.render_cams]
        self.cam_writers = [make_writer(self.config.vid_output_dir / cam.name, cam.width, cam.height, cam.fps) for cam in self.config.render_cams]
        self.steps_per_frames = [int(1 / cam.fps / self.model.opt.timestep) for cam in self.cam_renderers]
    ...
    def collect_camera(self, data: mujoco.MjData) -> list[torch.Tensor]:
        ...
        for i in range(len(self.obs_renderers)):
            self.obs_renderers[i].update_scene(data, camera=self.config.render_cams[i].name)
    ...
    def step(self, actions: list[torch.Tensor] | torch.Tensor, env_idx: int = -1, step_n: int = None) -> EnvObservation | list[EnvObservation]:
        if env_idx == -1:
            observations = []
            for i in range(len(self.datas)):
        ...
        for _ in range(self.steps_per_control):
            mujoco.mj_forward(self.model, self.datas[env_idx])
